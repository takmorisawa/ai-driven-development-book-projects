...
<div class="bg-orange-50 min-h-screen p-4">
  ...
  {#if $showModal}
    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white p-4 rounded-lg shadow-xl w-1/3 border-2 border-orange-200">
        <h2 class="text-xl mb-4 text-orange-600 font-bold">新規地形追加</h2>
        <label class="block mb-2 text-gray-700">
          地形名:
          <input type="text" bind:value={newTerrainName} class="border-2 border-orange-200 p-2 w-full bg-white text-gray-800 rounded focus:border-orange-400 focus:outline-none" />
        </label>
        <label class="block mb-2 text-gray-700">
          説明:
        ...
        <div class="flex justify-end">
          ...
          <button class="bg-orange-400 text-white p-2 rounded hover:bg-orange-500 transition-colors font-semibold shadow-sm" on:click={addTerrain}>追加</button>
        </div>
      </div>
    </div>
  {/if}
  ...
  {#if $showEditModal}
    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white p-4 rounded-lg shadow-xl w-1/3 border-2 border-orange-200">
        <h2 class="text-xl mb-4 text-orange-600 font-bold">地形情報編集</h2>
        <label class="block mb-2 text-gray-700">
          地形名:
          <input type="text" bind:value={editTerrainName} class="border-2 border-orange-200 p-2 w-full bg-white text-gray-800 rounded focus:border-orange-400 focus:outline-none" />
        </label>
        <label class="block mb-2 text-gray-700">
          説明:
        ...
        <div class="flex justify-end">
          ...
          <button class="bg-orange-400 text-white p-2 rounded hover:bg-orange-500 transition-colors font-semibold shadow-sm" on:click={editTerrain}>保存</button>
        </div>
      </div>
    </div>
  {/if}
  ...
  {#if $terrains.length === 0}
    <p class="text-center text-gray-600">地形が登録されていません</p>
  {:else}
    <table class="min-w-full bg-white text-gray-800 shadow-md rounded-lg border-2 border-orange-200">
      <thead>
        <tr class="bg-orange-100">
          <th class="py-2 px-4 border-b border-orange-200 text-orange-700 font-bold">地形名</th>
      ...
      <tbody>
        {#each $terrains as terrain}
          <tr class="hover:bg-orange-50 transition-colors">
            <td class="py-2 px-4 border-b border-orange-200">{terrain.name}</td>
            <td class="py-2 px-4 border-b border-orange-200">{terrain.region.name}</td>
            <td class="py-2 px-4 border-b border-orange-200">{terrain.latitude}</td>
            <td class="py-2 px-4 border-b border-orange-200">{terrain.longitude}</td>
