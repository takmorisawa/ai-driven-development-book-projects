...
- **地図ライブラリ**：日本地図の表示には適切な地図ライブラリ（Leaflet、Google Maps API、OpenStreetMapなど）を使用する
...
- **UIコンポーネント**：
  - 地域名: 地域の名前を表示します。
  - 都道府県名: 都道府県名を表示します。
  - 名産品数: その地域の名産品の数を表示します。
  - リンク: 地域詳細ページへのリンクを設定します。

### 6.0.4 名産品カード
- **概要**: 名産品の情報を表示するカード
- **プロパティ**：
  - `product`: 名産品オブジェクト（`Product`型）
- **UIコンポーネント**：
  - **名産品画像**: 名産品の画像を表示します。
  - **名産品名**: 名産品の名前を表示します。
  - **地域名**: 名産品の産地の地域名を表示し、地域詳細ページへのリンクを設定します。
  - **生産量ランキング**: 生産量ランキング（ベスト3など）を表示します。
  - **旅行プラン追加ボタン**: 名産品の産地を旅行プランに追加するためのボタン。クリックイベントで旅行プランへの追加を行います。
  - **旅行プラン状態の監視**: 旅行プランへの追加状態を監視し、UIに反映します。

### 6.0.5 名所カード
- **概要**: 名所の情報を表示するカード
- **プロパティ**：
  - `landmark`: 名所オブジェクト（`Landmark`型）
- **UIコンポーネント**：
  - **名所画像**: 名所の画像を表示します。
  - **名所名**: 名所の名前を表示します。
  - **地域名**: 名所の所在地域名を表示し、地域詳細ページへのリンクを設定します。
  - **説明**: 名所の簡単な説明を表示します。
  - **旅行プラン追加ボタン**: 名所を旅行プランに追加するためのボタン。クリックイベントで旅行プランへの追加を行います。
  - **旅行プラン状態の監視**: 旅行プランへの追加状態を監視し、UIに反映します。

### 6.0.6 地形カード
- **概要**: 地形の情報を表示するカード
- **プロパティ**：
  - `terrain`: 地形オブジェクト（`Terrain`型）
- **UIコンポーネント**：
  - **地形画像**: 地形の画像を表示します。
  - **地形名**: 地形の名前（リアス式海岸、扇状地など）を表示します。
  - **地域名**: 地形の所在地域名を表示し、地域詳細ページへのリンクを設定します。
  - **説明**: 地形の特徴や説明を表示します。
...
- **API**：
  ...
  - PUT /admin/api/regions/[id] 地域の更新
- **UIコンポーネント**：
  - **地域一覧テーブル**：
    - 地域名
    - 都道府県名
    - 登録日
    - 名産品管理ページへのリンク(/admin/regions/[id]/products)
    - 名所管理ページへのリンク(/admin/regions/[id]/landmarks)
    - 地形管理ページへのリンク(/admin/regions/[id]/terrains)
    - 編集ボタン
...
- **API**：
  ...
  - PUT /admin/api/products/[id] 名産品情報の更新
- **UIコンポーネント**：
  - **名産品一覧テーブル**：
    - 名産品名
    - 地域名
...
- **API**：
  ...
  - PUT /admin/api/landmarks/[id] 名所情報の更新
- **UIコンポーネント**：
  - **名所一覧テーブル**：
    - 名所名
    - 地域名
...
- **API**：
  ...
  - PUT /admin/api/terrains/[id] 地形情報の更新
- **UIコンポーネント**：
  - **地形一覧テーブル**：
    - 地形名
    - 地域名
...
### 7.1.1 地図初期化機能
- **関数名**: `initMap`
- **引数**: `containerId: string, options: MapOptions`
- **説明**: 指定されたコンテナに日本地図を初期化します。地図ライブラリを使用して地図を表示します。

### 7.1.2 マーカー追加機能
- **関数名**: `addMarker`
- **引数**: `marker: Marker`
- **説明**: 地図上にマーカーを追加します。マーカーの種類（名産品、名所、地形）に応じてアイコンを変更します。
...
- **関数名**: `setFilter`
- **引数**: `filter: MapFilter`
- **説明**: マーカーの表示/非表示をフィルターで制御します。

### 7.1.7 ストア
- **`mapInstance`**: 地図インスタンスを保持するSvelteのwritableストア。
- **`markers`**: 地図上のマーカー一覧を保持するSvelteのwritableストア。
- **`currentFilter`**: 現在のフィルター設定を保持するSvelteのwritableストア。
- **`selectedMarker`**: 現在選択中のマーカーを保持するSvelteのwritableストア。
...
- **説明**: 指定された地域IDまたは名所IDを旅行プランに追加します。ローカルストレージに保存されます。
...
- **関数名**: `getTravelPlan`
- **引数**: なし
- **説明**: ローカルストレージから旅行プランリストを取得します。

### 7.2.4 ストア
- **`travelPlanItems`**: 旅行プランの項目リストを保持するSvelteのwritableストア。
